...
public class ProductService {
    ...
    public Optional<Product> getProductById(Long id) { //Optional serve para conferir se existe um valor ou não na lista procurada
        return products.stream()
            .filter(p -> p.getId().equals(id)) //equivalente a for each
            .findFirst(); 
    }
    ...
    public Optional<Product> updateProduct(Long id, ProductRequestUpdate dto) {
        ...
        return products.stream()
            ...
            .findFirst()
            .map(p -> { // atualiza o valor encontrado
